...
\n## Quyền xem và chỉnh sửa\n- Chưa đăng nhập: chỉ xem; mọi thao tác POST đều yêu cầu đăng nhập.\n- Mật khẩu ban đầu lấy từ biến môi trường `EDIT_PASSWORD`; nếu chưa đặt, mật khẩu tạm là `DoiMatKhauNgay123!`.\n- Đăng nhập tại `/login`, sau đó đổi mật khẩu ở nút **Đổi mật khẩu** (ít nhất 10 ký tự).\n- Hãy đặt `SECRET_KEY` riêng trong Render. Đây là lớp bảo vệ cơ bản theo phiên trình duyệt.\n
\n## Ghi nhớ đăng nhập\n- Trang đăng nhập có lựa chọn **Ghi nhớ đăng nhập trên thiết bị này**. Khi bật, trình duyệt giữ phiên đăng nhập tối đa 30 ngày bằng cookie phiên được ký; không lưu mật khẩu dạng văn bản trong trình duyệt.\n- Chỉ bật trên thiết bị cá nhân, không bật trên máy dùng chung. Chọn **Thoát sửa** để kết thúc phiên.\n- Trên Render cần đặt `SECRET_KEY` ngẫu nhiên, riêng tư. `COOKIE_SECURE=1` dùng HTTPS (mặc định).\n- Đây chưa phải đăng nhập Face ID/vân tay (Passkey/WebAuthn). Muốn dùng sinh trắc học thật cần triển khai đăng ký khóa trên từng thiết bị và xác minh WebAuthn ở máy chủ; không giả lập bằng cách lưu ảnh/vân tay.\n